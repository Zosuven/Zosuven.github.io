...
<span style="color:purple">in the diagram *1.1-2* , the height $h$ is exaggerated(actually $R>>>h$ not the ratio</span>
...
for the given Scenario, the Earth's radius is $R \approx 6371 \operatername{km}$,

and we assume a $2 \operatername{m}$ tall sailor oberving from central mast on the "Dar Mlodziezy sailing" ship,

for which the height of the central mast is $197 \operatername{ft}$ above the deck, or about $66 \operatername{m}$
above water.

thus $h= 66+2= 68 \operatername{m}$, so the horizon is approximately $sqrt{2\times 6371000\times 68} \approx 29436 \operatername{m}$, or $29.4\operatername{km}$ distant.
